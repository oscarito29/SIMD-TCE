...
def monitoreo():
    ...
    for p in pacientes:
        ...
        if prediccion <= 8:
            ...
            porcentaje = 90
            recomendaciones = [
                "Evaluar presión intracraneal inmediatamente.",
                "Solicitar TAC de control urgente.",
                "Monitorear signos vitales cada 30 minutos."
            ]
        elif prediccion <= 12:
            nivel = "Moderado"
            color = "#f1c40f"
            ...
            porcentaje = 60
            recomendaciones = [
                "Continuar observación en UCI intermedia.",
                "Controlar presión arterial y temperatura.",
                "Evaluar respuesta neurológica cada 2 horas."
            ]
        else:
            nivel = "Estable"
            color = "#2ecc71"
            ...
            recomendaciones = [
                ...
                "Revisar evolución diaria.",
                "Actualizar registro médico."
            ]

        factores = [
            {"nombre": "Escala Glasgow", "valor": datos_paciente["escala_glasgow_ingreso"], "impacto": "Muy alto" if prediccion < 10 else "Moderado"},
            {"nombre": "Tipo de lesión", "valor": datos_paciente["tipo_lesion"], "impacto": "Alto" if "fractura" in datos_paciente["tipo_lesion"].lower() else "Bajo"},
            {"nombre": "Complicaciones", "valor": datos_paciente["complicaciones"], "impacto": "Moderado" if datos_paciente["complicaciones"] != "Ninguna" else "Bajo"}
        ]

        # 🔹 Corregido: campos que espera React
        alertas.append({
            "paciente": p["nombre"],
            "dni": p["dni"],
            "nivel_riesgo": nivel,
            "descripcion": f"Paciente con evolución {nivel.lower()} detectada por IA.",
            "color": color,
            "icono": icono,
